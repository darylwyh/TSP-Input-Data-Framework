...
class StateTSP(NamedTuple):
    # NamedTuple, immutable simple, immutable class 
    # Fixed input
    loc: torch.Tensor
    dist: torch.Tensor
    ...
    def visited(self):
        if self.visited_.dtype == torch.uint8:
            ...
            return mask_long2bool(self.visited_, n=self.loc.size(-2)) # access the second-to-last dimension

    # Indexing StateTSP object
    ...
    def initialize(loc, visited_dtype=torch.uint8):
        ...
        return StateTSP(
            loc=loc,
            # Computes the pairwise distances between points using broadcasting and norm
            # [batch_size, num_points, num_points, dimensions]
    ...
    def all_finished(self):
        # Compares the current step i with the number of locations
        # Exactly n steps
